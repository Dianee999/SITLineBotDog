...
def select_fd_stc():
    ...
    try:
        ...
        today_str2 = today2.strftime('%Y%m%d')

        s3.Bucket('lab-385515').download_file(f'{today_str2}主動投信.csv', f'/tmp/{today_str2}主動投信.csv')
        df_fd_d = pd.read_csv(f'/tmp/{today_str2}主動投信.csv', encoding='utf-8')
        ...
        # 取得投信買賣超資料(年)
        s3.Bucket('lab-385515').download_file('merged/2021.12.15至2022主動投信(時間排序).csv', '/tmp/2021.12.15至2022主動投信(時間排序).csv')
        df_fd_y = pd.read_csv('/tmp/2021.12.15至2022主動投信(時間排序).csv', encoding='utf-8', parse_dates=['Date'])
...
def lambda_handler(event, context):
    ...
    try:
        today_str2 = today2.strftime('%Y-%m-%d')
        table_name = f'{str(today_str2)}-message'
        ...
        table2.meta.client.get_waiter('table_exists').wait(TableName=table_name)
        ...
        # 開盤   
            ...
            if len(all_transactions) == 0:
                message =  f"{str(today_str2)}\n明天沒有推薦買賣的證券喔\n"
                item2 = {
                        'message': message
                    }
                table2.put_item(Item=item2)
                textMessage = TextSendMessage(text = message)
                line_bot_api.multicast(user_ids, textMessage)
            else:
